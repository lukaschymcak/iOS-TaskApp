    ...
    var body: some View {
        NavigationStack{
            ...
            VStack{
                    ...
                    ScrollView(.horizontal,showsIndicators: false){
                        ScrollViewReader { proxy in
                            HStack(spacing: 15){
                                ...
                                ForEach(plantsModule.wateringLocations.sorted(by: { $0.key.id < $1.key.id }),id: \.key) { location,number  in
                                            ...
                                            ZStack{
                                                if selectedLocation == location{
                                                    RoundedRectangle(cornerRadius: 20)
                                                        .stroke(plantsModule.color,lineWidth: 5)
                                                        .frame(height: 50)
                    ...
                    .frame(maxWidth: .infinity,alignment: .center)
                plantsModule.color.frame(width: GeometryProxy.size.width - 30, height: 5)
                    .clipShape(.rect(cornerRadius: 20))
                VStack{
             
                        PlantListView(plantsModule: plantsModule, location: selectedLocation)
